...
export default function DriverLayout() {
  ...
  const syncFromDelivery = useDriverStore((s) => s.syncFromDeliveryStore)
  ...
  useEffect(() => {
    const sync = () => {
      ...
      if (user?.id) {
        const routes = useDeliveryStore.getState().routes
        const stops = useDeliveryStore.getState().stops
        syncFromDelivery(user.id, routes, stops, orders)
      }
    }
    ...
    const unsubDelivery = useDeliveryStore.subscribe(() => sync())
  ...
  }, [syncReadyOrders, syncFromDelivery, user?.id])
